...
func checkUpdate() {
	...
	fmt.Printf("新しいバージョンがリリースされています (New version released): v%s -> v%s\n", pjsekaioverlay.Version, latestVersion)
	fmt.Printf("ダウンロード (Download Here) -> %s\n", release.GetHTMLURL())
}
...
func origMain(isOptionSpecified bool) {
	...
	if !skipAviutlInstall {
		...
		if success {
			fmt.Println("AviUtlオブジェクトのインストールに成功しました。(AviUtl object successfully installed.)")
		}
	}
...
func main() {
	...
	if !isOptionSpecified {
		fmt.Print(color.CyanString("\n何かキーを押すと終了します... | Press any key to exit..."))
